...
/*
 * Sử dụng lại walk_cs_node từ maapi.c bằng cách gọi
 * maapi_load_schema (hàm public đã có).
 * Ở đây ta wrap lại cho tiện dùng với maapi_session_t.
 */
bool maapi_load_schema_into(maapi_session_t *m, schema_node_t **out_schema) {
    if (!out_schema) return false;

    /* Tạo CLI session tạm để dùng maapi_load_schema */
    cli_session_t tmp;
    memset(&tmp, 0, sizeof(tmp));
    tmp.schema = schema_new_node("__root__");
    if (!tmp.schema) return false;

    /* Ghi env để maapi_load_schema dùng đúng host:port */
    char port_str[16];
    snprintf(port_str, sizeof(port_str), "%d", m->port);
    setenv("CONFD_IPC_ADDR", m->host, 1);
    setenv("CONFD_IPC_PORT", port_str, 1);

    bool ok = maapi_load_schema(&tmp);
    if (ok) {
        schema_free(*out_schema);
        *out_schema = tmp.schema;
    } else {
        schema_free(tmp.schema);
    }
    return ok;
}
