/*
 * main-maapi.c — CLI direct mode qua ConfD MAAPI (không cần NETCONF)
 *
 * Kết nối thẳng vào ConfD IPC port (mặc định :4565).
 * Toàn bộ get-config / set / commit đi qua MAAPI — không mở TCP/SSH.
 *
 * Build:
 *   make WITH_MAAPI=1 CONFD_DIR=/path/to/confd maapi-direct
 *   → c/cli-netconf-c-maapi
 *
 * Chạy:
 *   CONFD_IPC_ADDR=127.0.0.1 CONFD_IPC_PORT=4565 ./cli-netconf-c-maapi
 *
 * Env vars:
 *   CONFD_IPC_ADDR   ConfD host    (default: 127.0.0.1)
 *   CONFD_IPC_PORT   ConfD port    (default: 4565)
 *   MAAPI_USER       user label    (default: admin)
 *   NE_NAME          label prompt  (default: confd)
 *
 * Commands:
 *   show running-config [path...]
 *   show candidate-config [path...]
 *   set <keypath> <value>          — set leaf (keypath kiểu /a/b/c)
 *   set                            — paste XML config (kết thúc bằng dòng trống)
 *   unset <keypath>                — xoá node
 *   commit
 *   validate
 *   discard
...
#include <stdio.h>
int main(void) {
    fprintf(stderr, "This binary requires ConfD MAAPI.\n"
                    "Rebuild: make WITH_MAAPI=1 CONFD_DIR=/path/to/confd maapi-direct\n");
    return 1;
}
